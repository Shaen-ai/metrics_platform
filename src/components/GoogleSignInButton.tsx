...
export function GoogleSignInButton({ label, className = "" }: Props) {
  return (
    <button
      type="button"
      onClick={() => {
        window.location.href = getGoogleOAuthRedirectUrl("admin");
      }}
      className={`flex w-full items-center justify-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--card)] px-4 py-2.5 text-sm font-semibold text-[var(--foreground)] transition-colors hover:bg-[var(--background)] cursor-pointer ${className}`}
    >
